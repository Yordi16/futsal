...
@section('content')
    {{-- CDN SweetAlert diletakkan di dalam section agar pasti terload --}}
    ...
    <div class="space-y-8">
        ...
        <div class="flex items-center gap-6 p-6 bg-slate-50 rounded-[2rem] border border-slate-100">
            ...
            <div>
                ...
                <p class="text-emerald-600 font-bold text-lg">Rp {{ number_format($lapangan->harga_per_jam, 0, ',', '.') }}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            @forelse($jadwals as $jadwal)
                ...
                <div class="relative group">
                    <div
                        class="p-6 rounded-3xl border-2 transition-all duration-300 
                    {{ $jadwal->status_slot === 'tersedia'
                        ...
                        <div class="flex justify-between items-start mb-4">
                            <div
                                class="w-10 h-10 rounded-xl flex items-center justify-center {{ $jadwal->status_slot === 'tersedia' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-400' }}">
                                <i class="fas fa-clock"></i>
                            </div>
                            <span
                                class="text-[10px] font-black uppercase px-3 py-1 rounded-lg {{ $jadwal->status_slot === 'tersedia' ? 'bg-emerald-500 text-white' : 'bg-slate-300 text-slate-600' }}">
                                {{ $jadwal->status_slot }}
                            </span>
                        </div>

                        <h5 class="text-xl font-black text-slate-800">{{ $jadwal->jam_mulai }} - {{ $jadwal->jam_selesai }}
                        </h5>
                        <p class="text-slate-500 text-xs font-medium mb-1">Durasi: {{ $jadwal->durasi }} Jam</p>

                        {{-- Tampilan Harga yang dikirim dari Controller --}}
                        <p class="text-emerald-600 font-bold text-sm mb-6">
                            Harga: Rp {{ number_format($jadwal->total_harga, 0, ',', '.') }}
                        </p>
                        ...
                        @if ($jadwal->status_slot === 'tersedia')
                            <form id="form-booking-{{ $jadwal->id }}" action="/user/booking" method="POST">
                                ...
                                <button type="button"
                                    ...
                                    class="w-full py-3 bg-indigo-950 text-white rounded-xl font-bold text-sm hover:bg-emerald-500 transition-all uppercase tracking-widest shadow-lg shadow-indigo-950/20">
                                    Booking Slot
                                </button>
                            </form>
                        @else
                            <button disabled
                                class="w-full py-3 bg-slate-200 text-slate-400 rounded-xl font-bold text-sm uppercase tracking-widest cursor-not-allowed">
                                Sudah Terisi
                            </button>
                        @endif
                    </div>
            ...
            @empty
                <div
                    class="col-span-full py-20 text-center bg-slate-50 rounded-[3rem] border border-dashed border-slate-300">
    ...
    <script>
        function confirmBooking(id, mulai, selesai) {
            Swal.fire({
                ...
                text: "Anda akan memesan jadwal jam " + mulai + " - " + selesai + ". Lanjutkan?",
                icon: 'question',
