    ...
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <h1 class="text-3xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                    <i class="fas fa-chart-line text-emerald-500"></i> Analitik & Laporan
                </h1>
                <p class="text-slate-500 font-medium mt-1">Data pendapatan berdasarkan booking yang berhasil dikonfirmasi.</p>
            </div>

            <div class="flex items-center gap-3">
                {{-- Tombol Export dengan Query String agar filter tanggal ikut terbawa --}}
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
            <form action="{{ route('admin.report.index') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                <div class="flex-1">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 mb-1 block">Tanggal Mulai</label>
                    <input type="date" name="start_date" value="{{ $start }}" 
                        class="w-full bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 font-bold text-slate-700">
                </div>
                <div class="flex-1">
                    <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-2 mb-1 block">Tanggal Selesai</label>
                    <input type="date" name="end_date" value="{{ $end }}" 
                        class="w-full bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-emerald-500 font-bold text-slate-700">
                </div>
                <button type="submit" class="px-8 py-3 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-900 transition-all">
                    Filter Data
                </button>
                @if($start || $end)
                    <a href="{{ route('admin.report.index') }}" class="px-4 py-3 text-slate-400 hover:text-rose-500 font-bold transition-all">
                        Reset
                    </a>
                @endif
            </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                ...
                <h3 class="text-3xl font-black text-slate-800">Rp {{ number_format($total->sum('total'), 0, ',', '.') }}</h3>
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <div class="px-10 py-8 border-b border-slate-50 flex items-center justify-between">
                ...
                <span class="text-[10px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 px-3 py-1 rounded-lg uppercase">
                    {{ $start ? \Carbon\Carbon::parse($start)->format('d M') : 'Awal' }} - {{ $end ? \Carbon\Carbon::parse($end)->format('d M') : 'Sekarang' }}
                </span>
            </div>
            <div class="p-10">
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-400">
                            ...
                            <th class="px-8 py-5 text-center text-[10px] font-black uppercase tracking-widest">Waktu Main</th>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-8 py-5 text-center">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-slate-600">{{ $booking->jadwalLapangan->tanggal ?? '-' }}</span>
                                        <span class="text-[10px] font-bold text-emerald-500 uppercase">{{ $booking->jadwalLapangan->jam_mulai }} - {{ $booking->jadwalLapangan->jam_selesai }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 text-right">
                                    <span class="font-black text-emerald-600">Rp {{ number_format($booking->total_harga, 0, ',', '.') }}</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-12 text-slate-400 font-bold uppercase tracking-widest text-xs">Belum ada transaksi sukses</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        grid: { strokeDashArray: 5, color: '#f1f5f9' },
                        ticks: {
